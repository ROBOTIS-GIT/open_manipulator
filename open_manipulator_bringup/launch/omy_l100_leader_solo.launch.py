...
#
# Single-arm gravity-compensation bringup (no /leader namespace, no frame
# prefix) so the real robot can be moved by hand while rviz mirrors it
# using the default, unprefixed topic and frame names.
...
def generate_launch_description():
    ...
    declared_arguments = [
        ...
        DeclareLaunchArgument(
            'urdf_file_name',
            default_value='omy_l100.urdf.xacro',
            description='Xacro file to use: omy_l100.urdf.xacro (original) or '
                        'custom_omy_l100.urdf.xacro (shortened links)',
        ),
    ]
    ...
    # Generate URDF file using xacro (original or custom, selected via urdf_file_name)
    urdf_file = Command([
    ...
    # Reuse the leader_ai controller config (gravity compensation / spring actuator)
