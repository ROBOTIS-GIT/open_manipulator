...
from launch_param_builder.utils import load_xacro
from moveit_configs_utils import MoveItConfigsBuilder


HANDS_DESCRIPTION_XACRO = Path(
    get_package_share_directory('open_manipulator_description')
) / 'urdf' / 'omy_3m' / 'omy_3m_hands.urdf.xacro'
...
def generate_launch_description():
    ...
    moveit_config = (
        moveit_builder.robot_description_semantic(
            Path('config') / 'omy_3m_hands' / 'omy_3m_hands.srdf'
        )
        .joint_limits(Path('config') / 'omy_3m_hands' / 'joint_limits.yaml')
        .trajectory_execution(Path('config') / 'omy_3m_hands' / 'moveit_controllers.yaml')
