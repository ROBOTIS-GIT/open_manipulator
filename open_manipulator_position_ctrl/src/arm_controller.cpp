...
void ArmController::initPublisher(bool using_gazebo)
{
  ...
  gripper_command_pub_ = nh_.advertise<std_msgs::String>(robot_name_ + "/gripper", 10);
...
void ArmController::initSubscriber(bool using_gazebo)
{
  ...
  teleop_key_cmd_val_sub_ = nh_.subscribe("/open_manipulator/teleop_key_cmd_val", 10,
                                            &ArmController::teleopKeyCmdValMsgCallback, this);
}
...
void ArmController::teleopKeyCmdValMsgCallback(const std_msgs::String::ConstPtr &msg)
{
  ros::AsyncSpinner spinner(1);
  spinner.start();
  ...
  open_manipulator_msgs::KinematicsPose target_pose;
  target_pose.group_name = "arm";
  target_pose.max_accelerations_scaling_factor = 0.0;
  target_pose.max_velocity_scaling_factor = 0.0;
  target_pose.tolerance = 0.01;
  target_pose.pose = move_group->getCurrentPose().pose;
  
  if(cmd == 'q')
  {
    ROS_INFO("Increase x axis in cartesian space using teleop");
    target_pose.pose.position.x += 0.05;
    calcPlannedPath(target_pose);
  }
  else if(cmd == 'w')
  {
    ROS_INFO("Decrease x axis in cartesian space using teleop");
    target_pose.pose.position.x -= 0.05;
    calcPlannedPath(target_pose);
  }
  else if(cmd == 'a')
  {
    ROS_INFO("Increase y axis in cartesian space using teleop");
    target_pose.pose.position.y += 0.01;
    calcPlannedPath(target_pose);
  }
  else if(cmd == 's')
  {
    ROS_INFO("Decrease y axis in cartesian space using teleop");
    target_pose.pose.position.y -= 0.01;
    calcPlannedPath(target_pose);
  }
  else if(cmd == 'z')
  {
    ROS_INFO("Increase z axis in cartesian space using teleop");
    target_pose.pose.position.z += 0.03;
    calcPlannedPath(target_pose);
  }
  else if(cmd == 'x')
  {
    ROS_INFO("Decrease z axis in cartesian space using teleop");
    target_pose.pose.position.z -= 0.03;
    calcPlannedPath(target_pose);
  }
  else if(cmd == 'h')
  {
    ROS_INFO("go home position");
    open_manipulator_msgs::JointPosition msg;
  ...
  else if(cmd == 'b')
  {
    ...
    msg.position.push_back(0.0);
    msg.position.push_back(-60 * 3.14 / 180);
    msg.position.push_back( 20 * 3.14 / 180);
    msg.position.push_back( 40 * 3.14 / 180);
...
void ArmController::demo(void)
{
  struct timeval end;
  static int demo_count_ = -1;
  static int wait_time_ = 3;
  ...
  if(wait_time_ <= end.tv_sec - start.tv_sec)
  {
    msg.joint_name.push_back("joint1");
    msg.joint_name.push_back("joint2");
    msg.joint_name.push_back("joint3");
    ...
    case 0:
      msg.position.push_back(0.0);
      msg.position.push_back(-0.92);
      msg.position.push_back(0.33);
      msg.position.push_back(0.69);
      calcPlannedPath(msg);
      wait_time_ = 4;
      gettimeofday(&start, NULL);
    break;
    case 1:
      gripper_msg.data = "grip_on";
    ...
    case 6:
      ...
      msg.position.push_back(-0.75);
      msg.position.push_back(0.80);
      msg.position.push_back(-0.85);
      wait_time_ = 4;
      gettimeofday(&start, NULL);
      calcPlannedPath(msg);
    break;
  ...
  //else
    //ROS_INFO("%d",end.tv_sec - start.tv_sec);
}
...
int main(int argc, char **argv)
{
  ...
  gettimeofday(&controller.start, NULL);
